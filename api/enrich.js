...
5. 免费板块只做分析和描述,绝对不给任何建议、策略、推荐、指导
6. 禁止使用任何特殊符号(✦✧💎🔥→←★☆⭐●○等),只用纯文字和标点
7. 不要分析十神的过旺或缺失(如"缺木""有正财无偏财"等),只分析十神在四柱中的位置含义
8. 语气要像一位有30年经验的命理师在面对面交谈,温和专业,不要用条目式罗列,要用自然的段落叙述
...
=== 请生成(JSON格式) ===
{
  "personality": "性格特质深度分析(400-500字):从日主${chart.day_master}${chart.day_master_element}本质出发。逐一分析年干${chart.year_god}在年柱含义、月干${chart.month_god}在月柱含义、时干${chart.hour_god}在时柱含义。分析四柱合冲关系对性格的影响。分析早年(年柱)、青年(月柱)、中年(日柱)、晚年(时柱)的人生轨迹。要让人觉得被看透。",
